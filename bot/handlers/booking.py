...
async def start_booking(message: Message, session: AsyncSession, state: FSMContext) -> None:
    ...
    if not rooms:
        await message.answer("Комнаты пока не добавлены. Обратитесь к администратору.", reply_markup=main_menu())
        return
    await state.set_state(BookingStates.choosing_room)
    await message.answer("Выберите комнату:", reply_markup=rooms_keyboard(rooms, prefix="bookroom"))
    ...
    if action == "pick":
        ...
        if mode == "check_in":
            ...
            if check_out <= check_in:
                ...
                return
            if not await is_room_available(session, room_id, check_in, check_out):
...
async def show_availability_start(message: Message, session: AsyncSession) -> None:
    rooms = await get_active_rooms(session)
    if not rooms:
        await message.answer("Комнаты пока не добавлены.", reply_markup=main_menu())
        return
